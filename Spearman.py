...
# ==== 加载数据 ====
dataprocessor = DataProcessor(["/data/users/zzp/Trade/train_factor_1_smooth.json", "/data/users/zzp/Trade/train_factor_2_smooth.json"])
factorls = dataprocessor.get_random_factornames(k=-1)  # 使用全部特征
#X, y = dataprocessor.get_X_y(factorls)
X = dataprocessor.get_X(factorls)
y = dataprocessor.get_X("up_num")
y = y / 300
# 显示数据形状
...
# ==== 选出相关系数绝对值最高的前10个特征 ====
top_corrs = sorted(correlations, key=lambda x: abs(x[1]), reverse=True)[:30]
...
with open('features.json','w')as f:
    f.write(json.dumps(top_corrs[:30]))
